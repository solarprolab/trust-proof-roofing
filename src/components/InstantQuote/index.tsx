...
export default function InstantQuote() {
  ...
  const lookupRoofSize = async (address: string) => {
    ...
    try {
      ...
      let size: RoofSize;
      if (sqft < 1500) size = 'small';
      else if (sqft < 2500) size = 'medium';
      else if (sqft < 3500) size = 'large';
      else size = 'xlarge';
      setForm(f => ({ ...f, roofSize: size }));
      setRoofSqft(sqft);
    } catch {
          ...
          <div>
            ...
            {roofSqft && (
              <p className="text-xs text-green-600 font-medium mb-3 flex items-center gap-1.5">
                ...
                Roof detected: ~{roofSqft.toLocaleString()} sq ft — size pre-selected for you
              </p>
            )}
